...
if(instance_exists(obj_pointer)){
	...
	if (mp_grid_path(gridRoomTavern, playerPath, x, y, obj_pointer.x, obj_pointer.y, false)) {
		...
		//Checks if the pointer obj is inside the object instance
		if(instance_place(xClick, yClick, obj_table)) {
			//pointer object inside object table then store object into inst
			inst = instance_place(xClick, yClick, obj_table);
		} else if (instance_place(xClick, yClick, obj_tableWebbed)) {
			//pointer object inside object tableWebbed then store object into inst
			inst = instance_place(xClick, yClick, obj_tableWebbed);
		}
		...
		if (inst != noone) {
			...
			if (obj_player.x < inst.x) {
				if (obj_player.y < inst.y) {
					...
					obj_pointer.x = inst.x - 24;
				}
			} else {
				if (obj_player.y < inst.y) {
					obj_pointer.x = inst.x + inst.sprite_width + 24;
				} else {
