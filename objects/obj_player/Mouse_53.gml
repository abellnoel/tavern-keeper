...
if (distance_to_object(bubbleAtMouse) < serveDistance and instance_exists(bubbleAtMouse)) {
	customerSelected = instance_position(mouse_x, mouse_y, obj_customer);
	if (instance_exists(customerSelected)) { //only activates on customer bubbles
...
if (distance_to_object(itemAtMouse) < serveDistance and instance_exists(itemAtMouse)) {
	...
	if (itemAtMouse == inst_meatSource) {
		instance_destroy(serveIndicator); //destroy old serveIndicator
		serveState = SERVING.food;
		serveIndicator = instance_create_layer(x, y, "Instances", obj_foodBubble);
	}
	if (itemAtMouse == inst_aleSource) { 
		instance_destroy(serveIndicator);
		serveState = SERVING.drink;
		serveIndicator = instance_create_layer(x, y, "Instances", obj_drinkBubble);
	}
}